...
export default function AdminStudentsPage() {
  ...
  const fetchStudents = useCallback(async () => {
    setLoading(true);
    try {
      const bParam = branch === 'All Branches' ? '' : branch;
      ...
      <div className="glass-card p-5 rounded-2xl border space-y-4 shadow-md">
        <div className="flex items-center justify-between border-b border-slate-200/10 pb-3">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
            <Users className="h-4.5 w-4.5 text-indigo-500" /> Students Database Directory
          </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {/* Universal Search */}
          <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition-all duration-300 bg-slate-500/5 neon-border-focus`}>
        ...
        {(search || branch !== 'All Branches' || session !== 'All Sessions' || semester !== 'All Semesters') && (
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={10} className="py-20 text-center space-y-2">
                      ...
                      <p className="text-sm font-bold text-slate-400">No student records match selected filters.</p>
                      <button
                        onClick={handleClearFilters}
